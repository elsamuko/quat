...
int ParseFile( const char* nam, const char* _altpath, struct keyword_struct kwords[], char* ColBuf, int ColBufLen, int* ColBufCount,
    ...
    strcpy( name, nam );
    ConvertPath( name );
    strcpy( altpath, _altpath );
    ConvertPath( altpath );

    ini = fopen( name, "r" );

    if( ini == NULL ) {
        strcpy( fname, altpath );
        strcat( fname, name );
        ini = fopen( fname, "r" );
    ...
    while( !feof( ini ) && !( ferror_val = ferror( ini ) ) ) {
        ...
        if( i != -1 ) {
            ...
            for( i = 0; i < argcount; i++ ) {
                ...
                if( isint( &argint[i], &argument[i][0] ) == -1 ) {
                    argtype[i] += 1;
                }

                if( isdouble( &argdouble[i], &argument[i][0] ) == -1 ) {
                    argtype[i] += 2;
                }
...
/* rewrites file "name" */
{
    ...
    *png = fopen( pngname, "w+b" );
    ...
    if( zflag == 1 ) {
        png_info->width *= view.antialiasing;
    }
    ...
    if( zflag == 1 ) {
        png_info->height *= view.antialiasing;
    }
...
/* RC :  -3 No quat chunk; -4 No memory; -5 No Quat-PNG; -128 file ver higher than progver */
{
    ...
    frac = *f;
    view = *v;   /* Init to defaults. Important if filever<thisver */
    ...
    if( strncmp( ( char* )Buf, s, 4 ) != 0 ) {
        ...
        sprintf( Error, "%c%c%c%c", Buf[5], Buf[6], Buf[7], Buf[8] );
    }
    ...
    *ystart = ( ( Buf[strlen( s ) + 3] << 8 ) & 0xff00 ) | Buf[strlen( s ) + 4];
    ...
    for( i = 0; i < 4; i++ ) if( ReadNextDouble( &d, &frac.c[i], Error ) ) {
            return( -6 );
        }

    if( ReadNextDouble( &d, &frac.bailout, Error ) ) {
        return( -6 );
    }

    if( ReadNextInt( &d, &frac.maxiter, Error ) ) {
        return( -6 );
    }

    if( ReadNextDouble( &d, &frac.lvalue, Error ) ) {
        return( -6 );
    }

    if( ReadNextInt( &d, &frac.formula, Error ) ) {
        return( -6 );
    }
    ...
    if( ver >= 1.1 ) {
        for( j = 0; j < 4; ++j )
            for( i = 0; i < 4; ++i )
                if( ReadNextDouble( &d, &frac.p[j][i], Error ) ) {
                    return( -6 );
                }
    }

    for( i = 0; i < 3; i++ ) if( ReadNextDouble( &d, &view.s[i], Error ) ) {
            return( -6 );
        }

    for( i = 0; i < 3; i++ ) if( ReadNextDouble( &d, &view.up[i], Error ) ) {
            return( -6 );
        }

    for( i = 0; i < 3; i++ ) if( ReadNextDouble( &d, &view.light[i], Error ) ) {
            return( -6 );
        }

    if( ReadNextDouble( &d, &view.Mov[0], Error ) ) {
        return( -6 );
    }

    if( ReadNextDouble( &d, &view.Mov[1], Error ) ) {
        return( -6 );
    }

    if( ReadNextDouble( &d, &view.LXR, Error ) ) {
        return( -6 );
    }

    if( ReadNextInt( &d, &view.xres, Error ) ) {
        return( -6 );
    }

    if( ReadNextInt( &d, &view.yres, Error ) ) {
        return( -6 );
    }

    if( ReadNextInt( &d, &view.zres, Error ) ) {
        return( -6 );
    }

    if( ReadNextDouble( &d, &view.phongmax, Error ) ) {
        return( -6 );
    }

    if( ReadNextDouble( &d, &view.phongsharp, Error ) ) {
        return( -6 );
    }

    if( ReadNextDouble( &d, &view.ambient, Error ) ) {
        return( -6 );
    }

    if( ver >= 0.91 ) if( ReadNextInt( &d, &view.antialiasing, Error ) ) {
            return( -6 );
        }

    if( ver >= 0.92 ) if( ReadNextDouble( &d, &view.interocular, Error ) ) {
            return( -6 );
        }

    if( ReadNextInt( &d, &realpal->colnum, Error ) ) {
        return( -6 );
    }

    for( i = 0; i < realpal->colnum; i++ ) {
        if( ReadNextDouble( &d, &realpal->cols[i].weight, Error ) ) {
            return( -6 );
        }

        for( j = 0; j < 3; j++ ) if( ReadNextDouble( &d, &realpal->cols[i].col1[j], Error ) ) {
                return( -6 );
            }

        for( j = 0; j < 3; j++ ) if( ReadNextDouble( &d, &realpal->cols[i].col2[j], Error ) ) {
                return( -6 );
            }
    }
    ...
    do {
        if( ReadNextInt( &d, &cuttype, Error ) ) {
            return( -6 );
        }
        ...
        *( char* )&cutbuf[cutpos++] = ( char )cuttype;
        ...
        switch( cuttype ) {
            case CUT_PLANE:
                for( i = 0; i < 6; i++ ) if( ReadNextDouble( &d, &cutbuf[cutpos + i], Error ) ) {
                        return( -6 );
                    }
...
int PNGEnd( struct png_internal_struct* png_internal, unsigned char* buf, int actx, int acty ) {
    ...
    for( i = acty; i < ( int )png_internal->height; i++ ) {
...
int GetNextName( char* nowname, char* namewop ) {
    ...
    if( pathendp != NULL ) {
        pathlen = ( int )( pathendp - nowname ) + 1;
    } else {
        pathlen = 0;
        pathendp = nowname - 1;
    }
    ...
    strncpy( name, nowname, pathlen );
    name[pathlen] = 0;     /* copy path to name */
    ...
    if( namewop != NULL ) {
        sprintf( namewop, "%s%02i%s", onlyname, n - 1, onlyext );
    }
