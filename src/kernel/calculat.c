...
void GetObjectBefore( char* b, char* s, int p ) {
    ...
    */
    ...
    while( i >= 0 && strchr( operators, *( s + i ) ) == NULL ) {
        --i;
    }
...
void GetObjectBehind( char* b, char* s, int p ) {
    ...
    */
    ...
    while( strchr( operators, *( s + i ) ) == NULL && i <= strlen( s ) + 1 ) {
        i++;
    }
...
int DoTranslate( char* wrong, char* a, struct progtype* prog ) {
        ...
        if( ( bropen != -1 ) && ( brcount == 0 ) ) {
            ...
            if( fct != -1 ) {
                if( ( fct & 128 ) != 0 ) {
                    ...
                    switch( fct ) {
                        case 0:
                            k = DeclareFunction( "sin", f_sin, prog );
                            break;

                        case 1:
                            k = DeclareFunction( "cos", f_cos, prog );
                            break;

                        case 2:
                            k = DeclareFunction( "sqr", f_sqr, prog );
                            break;

                        case 3:
                            k = DeclareFunction( "sqrt", f_sqrt, prog );
                            break;

                        case 4:
                            k = DeclareFunction( "exp", f_exp, prog );
                            break;

                        case 5:
                            k = DeclareFunction( "ln", f_ln, prog );
                            break;

                        case 6:
                            k = DeclareFunction( "round", f_round, prog );
                            break;

                        case 7:
                            k = DeclareFunction( "trunc", f_trunc, prog );
                            break;

                        case 8:
                            k = DeclareFunction( "abs", f_abs, prog );
                            break;

                        case 9:
                            k = DeclareFunction( "tan", f_tan, prog );
                            break;

                        case 10:
                            k = DeclareFunction( "random", f_random, prog );
                            break;

                        case 11:
                            k = DeclareFunction( "atan", f_atan, prog );
                            break;

                        case 12:
                            k = DeclareFunction( "asin", f_asin, prog );
                            break;

                        case 13:
                            k = DeclareFunction( "acos", f_acos, prog );
                            break;
    ...
    for( i = 0; i < strlen( a ); i++ ) {
        if( strchr( prio3, a[i] ) != NULL ) {
            priori[i] = 3;
        } else if( strchr( prio2, a[i] ) != NULL ) {
            priori[i] = 2;
        } else if( strchr( prio1, a[i] ) != NULL ) {
            priori[i] = 1;
        }
    }
            ...
            if( priori[i] == ( char )j ) {
                ...
                if( ( IsNumber( obj1 ) == 1 ) || ( IsVar( obj1, prog ) == 1 ) ) {
                    ...
                    if( ( int )i - ( int )strlen( obj1 ) < 0 ) {
                        sprintf( wrong, "Internal error #3" );
                        FreeObjs( obj1, obj2, priori, suba );
                        return( 1 );
                    } else {
                        a[i - strlen( obj1 )] = registers[regscount];
                    }
                    ...
                    if( IsNumber( obj1 ) == 1 ) {
                        ...
                        prog->b[ocount] = &( prog->varvalues[VarNr( obj1, prog )] );
                    }
                    ...
                    if( ocount > 75 ) {
                        sprintf( wrong, "Formula too complex." );
                        FreeObjs( obj1, obj2, priori, suba );
                        return( 1 );
                    }
                ...
                switch( a[i] ) {
                    case '+':
                        prog->func[ocount] = f_add;
                        break;

                    case '-':
                        prog->func[ocount] = f_sub;
                        break;

                    case '*':
                        prog->func[ocount] = f_mul;
                        break;

                    case '/':
                        prog->func[ocount] = f_div;
                        break;

                    case '^':
                        prog->func[ocount] = f_pow;
                        break;
                }
                ...
                if( strlen( obj2 ) + strlen( obj1 ) > strlen( a ) ) {
                    sprintf( wrong, "Internal error #4" );
                    FreeObjs( obj1, obj2, priori, suba );
                    return( 1 );
                } else {
                    memset( a + i - strlen( obj1 ) + 1, ' ', strlen( obj2 ) + strlen( obj1 ) );
                }
