...
int iterate_0( struct iter_struct* is ) {
    ...
    while( ( is->exactiter == 0 && xss < is->bailout && iter < is->maxiter )
        ...
        iter++;
        olditer++;
        tmp = orbit[olditer][0] * orbit[olditer][1];
        orbit[iter][1] = tmp + tmp - is->c[1];
        tmp = orbit[olditer][0] * orbit[olditer][2];
        orbit[iter][2] = tmp + tmp - is->c[2];
        tmp = orbit[olditer][0] * orbit[olditer][3];
        orbit[iter][3] = tmp + tmp - is->c[3];
...
int iterate_1( struct iter_struct* is ) {
    ...
    orbit[0][0] = is->xstart[0];
    re2    = is->xstart[0] * is->xstart[0];
    orbit[0][1] = is->xstart[1];
    pure2  = is->xstart[1] * is->xstart[1];
    orbit[0][2] = is->xstart[2];
    pure2 += is->xstart[2] * is->xstart[2];
    orbit[0][3] = is->xstart[3];
    pure2 += is->xstart[3] * is->xstart[3];
...
int iternorm_1( point xstart, point c, vec3 norm, double bailout, int maxiter ) {
    ...
    while( xss < bailout && iter < maxiter ) {
        ...
        if( xss < 1E-16 ) {
            break;    /* go out of loop, normal vec3 will get zero; do approx. */
        }

        if( xss > 1E17 ) {
            break;    /* go out of loop before overflow; do approximate */
        }
...
int iterate_3( struct iter_struct* is ) {
    ...
    while( ( is->exactiter == 0 && re2 + pure2 < is->bailout && iter < is->maxiter )
        ...
        x2[0] = orbit[iter][0] * orbit[iter][0];
        re2    = x2[0];
        x2[1] = orbit[iter][1] * orbit[iter][1];
        pure2  = x2[1];
        x2[2] = orbit[iter][2] * orbit[iter][2];
        pure2 += x2[2];
        x2[3] = orbit[iter][3] * orbit[iter][3];
        pure2 += x2[3];
...
int DoCalcbase( struct basestruct* base, struct basestruct* sbase, struct view_struct* v,
    ...
    if( v->up[1]*base->z[2] - v->up[2]*base->z[1] == 0
            ...
            && v->up[0]*base->z[1] - v->up[1]*base->z[0] == 0 ) {
        return( -1 );
    }
    ...
    while( *( char* )&cutbuf[pos] != CUT_TERMINATOR  && flag == 0 ) {
        switch( *( char* )&cutbuf[pos] ) {
            case CUT_PLANE:
                ...
                if( scalarprod( n2, y1 ) > 0 ) {
                    sign1 = 1;
                } else {
                    sign1 = -1;
                }

                if( scalarprod( n2, y2 ) > 0 ) {
                    sign2 = 1;
                } else {
                    sign2 = -1;
                }
...
int newton( struct calc_struct* c, double zstart/*, int actiter*/, double boundary, double tdelta,
    ...
    while( fabs( xss1 ) > deltafn && conv > deltaconv ) {
        ...
        if( ta * taold <= 0 ) {
            return( -3 );    /* high- or low-point inbetween */
        }
...
float brightness( const vec3 ls, const vec3 p, const vec3 n, const vec3 z, double phongmax, double phongsharp,
    ...
    if( absolute == 0 ) {
        return( ( float )255 );    /* light source and point are the same! */
    }
    ...
    if( a > 0 ) {
        ...
        if( c < 0 ) {
            c = phongmax * exp( phongsharp * log( -c ) );
        } else {
            c = 0;
        }
    } else {
    ...
    if( a < 0 ) {
        ...
        if( result > 1.0 ) {
            result = 1.0;    /* Lambert and ambient together can't get bigger than 1 */
        }
        ...
        for( xa = 0; xa < c->v.antialiasing; xa++ ) {
            ...
            if( depth != c->v.zres ) {
                ...
                else */ /* use the ZBuffer */
                {
                    ...
                    if( x + xa > 0 ) {
                        dz2 = LBuf[( ( size_t )x + ( size_t )( ya + 1 ) * ( size_t )c->v.xres ) * ( size_t )c->v.antialiasing + ( size_t )( xa - 1 )] - depth;
                    } else {
                        dz2 = ( float )0;
                    }
