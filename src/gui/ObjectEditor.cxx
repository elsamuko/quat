...
void ObjectEditor::set( const frac_cpp& f ) {
    frac = f;
    maxiter->value( f.maxiter );
    maxiter->do_callback();
    lvalue->value( f.lvalue );
    lvalue->do_callback();
    bailout->value( f.bailout );
    bailout->do_callback();
    formula->value( f.formula );
    formula->do_callback(); // Also calls formula_changed()
    cre->value( f.c[0] );
    cre->do_callback();
    ci->value( f.c[1] );
    ci->do_callback();
