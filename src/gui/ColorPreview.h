...
class ColorPreview : public Fl_Valuator {
    ...
    public:
        ...
        void SetInputs( Fl_Button* add, Fl_Button* del, Fl_Value_Input* weight,
            ...
            _input_red1 = red1;
            _input_green1 = green1;
            _input_blue1 = blue1;
            _input_red2 = red2;
            _input_green2 = green2;
            _input_blue2 = blue2;
            _output_range = out;
            UpdateInputs();
        }
        ...
        void GetPal( struct realpal_struct& pal ) const {
            pal = _pal;
        }
        int current() const {
            return _idx;
        ...
        void add();
        void del();
        void weight( int pos, double v ) {
            if( pos > _idx ) {
                return;
            }

            _pal.cols[pos].weight = v;
            Update();
        }
        void red1( int pos, double v ) {
            if( pos > _idx ) {
                return;
            }

            _pal.cols[pos].col1[0] = v;
            Update();
        }
        void green1( int pos, double v ) {
            if( pos > _idx ) {
                return;
            }

            _pal.cols[pos].col1[1] = v;
            Update();
        }
        void blue1( int pos, double v ) {
            if( pos > _idx ) {
                return;
            }

            _pal.cols[pos].col1[2] = v;
            Update();
        }
        void red2( int pos, double v ) {
            if( pos > _idx ) {
                return;
            }

            _pal.cols[pos].col2[0] = v;
            Update();
        }
        void green2( int pos, double v ) {
            if( pos > _idx ) {
                return;
            }

            _pal.cols[pos].col2[1] = v;
            Update();
        }
        void blue2( int pos, double v ) {
            if( pos > _idx ) {
                return;
            }

            _pal.cols[pos].col2[2] = v;
            Update();
        }
    protected:
        virtual void draw();
